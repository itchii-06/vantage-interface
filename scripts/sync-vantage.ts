...
async function syncTypes(protocolPath: string, destRoot: string): Promise<void> {
  ...
  const tsFiles = await findFiles(typechainDir, (f) => f.endsWith(".ts"));
  const destTypesDir = path.join(destRoot, "types");
  let count = 0;

  for (const srcFile of tsFiles) {
    const relative = path.relative(typechainDir, srcFile);
    await fse.copy(srcFile, path.join(destTypesDir, relative));
    count++;
  }

  console.log(`  ✅ types/ — ${count} TypeScript file(s) copied`);
}
